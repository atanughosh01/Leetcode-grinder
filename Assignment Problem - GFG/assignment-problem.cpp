...
// Problem : https://practice.geeksforgeeks.org/problems/assignment-problem3016/1
...
public:
    ...
    void augment()  {// main function of the algorithm
        ...
        int q[31], wr = 0, rd = 0;  // q - queue for bfs, wr,rd - write and read pos in queue
        ...
        while (true) { // main cycle
            ...
            for (y = 0; y < n; y++) {
                ...
                // graph as a result of improving the labeling, we add edge
                // (slackx[y], y) to the tree if and only if !T[y] && slack[y]
                // == 0, also with this edge we add another one (y, yx[y]) or
                // augment the matching, if y was exposed
                if (!T[y] && slack[y] == 0) {
                    if (yx[y] == -1) { // exposed vertex in Y found - augmenting path exists!
                        ...
                        if (!S[yx[y]]) {
                            ...
                            add_to_tree(
                                yx[y],
                                slackx[y]);  // and add edges (x,y) and (y,yx[y]) to the tree
                        }
                    }
                }
            }
