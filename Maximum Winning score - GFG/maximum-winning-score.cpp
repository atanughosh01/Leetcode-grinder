...
// Problem : https://practice.geeksforgeeks.org/problems/4ead9c3991a3822f578309e2232bc5415ac35cb9/1
    ...
    long long findMaxScore(Node* root) {
        ...
        while(!q.empty()) {
            ...
            for(int i=0; i<sz; ++i) {
                ...
                if(node->left) {
                    long long temp = node->left->data*cur;
                    q.push({node->left,temp});
                }
                if(node->right) {
                    long long temp = node->right->data*cur;
