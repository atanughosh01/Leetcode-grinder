...
public:
    ...
    void push(int x) {
        ...
        if(max_frequency<freq_mp[x]) {
            max_frequency = freq_mp[x];
        }
    ...
    int pop() {
        ...
        // if the stack of maximum element becomes empty, 
        // then delete it from "freq_stack_mp" along with decreasing "max_frequency"
